...
FROM builder as fusesoc

RUN mkdir -p /tmp/build
COPY muntjac/python-requirements.txt /tmp/build 
RUN pip3 install -r /tmp/build/python-requirements.txt
...
ARG FUSESOC_COMMIT=main
LABEL fusesoc_commit=$FUSESOC_COMMIT
